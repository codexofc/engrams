//! `kept check` gives every frontmatter field a reader, and defines by what it
//! refuses the YAML subset the memory accepts.

use kept::check::{check_corpus, check_note};
use kept::note::Note;
...
fn messages(findings: &[kept::check::Finding]) -> String {
