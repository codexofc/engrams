...
fn main() {
    ...
    step("start");
    let e = engrams::embedder::Embedder::load(&model).unwrap();
    ...
    let idx = engrams::index::Index::load(&engrams::paths::state_dir(&root).join("index.bin")).expect("index missing, run `engram index`");
    step("index loaded");
