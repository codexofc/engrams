...
fn the_budget_is_derived_from_the_model_window() {
    assert_eq!(kept::chunking::budget_for(512), 450);
    assert_eq!(kept::chunking::budget_for(8192), 8130);
}

#[test]
fn a_tiny_window_still_leaves_room_for_content() {
    assert!(kept::chunking::budget_for(64) > 0);
}
