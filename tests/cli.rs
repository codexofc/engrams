...
fn root(name: &str) -> Root {
    let r = std::env::temp_dir().join(format!("engram-cli-{name}-{}", std::process::id()));
...
fn engram(root: &Path, args: &[&str], stdin: &str) -> (bool, String, String) {
    let mut child = Command::new(env!("CARGO_BIN_EXE_engram"))
        .args(args)
        .env("ENGRAM_ROOT", root)
        .env("ENGRAM_MODEL", root.join("no-model"))
...
fn write_creates_a_conforming_note_and_refuses_secrets_and_collisions() {
    let r = root("write");
    let (ok, out, _) = engram(&r.0, &["write", "fam/proj", "gamma", "--type", "feedback", "--description", "rule: never do X"], "The body of gamma.\n");
    assert!(ok, "{out}");
    ...
    let (ok, _, err) = engram(&r.0, &["write", "fam/proj", "gamma", "--type", "feedback", "--description", "d"], "again");
    assert!(!ok && err.contains("already exists"), "{err}");
    let (ok, _, err) = engram(&r.0, &["write", "fam/proj", "delta", "--type", "project", "--description", "d"], "token glpat-ABCDEFGHIJKLMNOPQRS here");
    assert!(!ok && err.contains("secret"), "{err}");
    assert!(!r.0.join("fam/proj/delta.md").exists());
    let (ok, _, err) = engram(&r.0, &["write", "fam/proj", "Bad_Name", "--type", "project", "--description", "d"], "body");
...
fn supersede_archives_the_old_note_and_rewrites_links_to_it() {
    ...
    let (ok, out, err) = engram(&r.0, &["supersede", "beta", "beta-2", "--type", "reference", "--description", "the beta note, version 2"], "New body.");
    assert!(ok, "{out}{err}");
...
fn link_adds_a_cross_reference_both_ways_once() {
    let r = root("link");
    let (ok, _, err) = engram(&r.0, &["link", "alpha", "beta"], "");
    assert!(ok, "{err}");
    ...
    let (ok, _, _) = engram(&r.0, &["link", "alpha", "beta"], "");
    assert!(ok);
...
fn since_and_why_read_git_history() {
    let r = root("git");
    engram(&r.0, &["append", "alpha"], "Tracked addition.");
...
fn check_and_secrets_pass_on_a_clean_root() {
    let r = root("check");
    let (ok, out, _) = engram(&r.0, &["check"], "");
    assert!(ok, "{out}");
    let (ok, _, _) = engram(&r.0, &["secrets"], "");
    assert!(ok);
}
...
fn learn_derives_missed_reads_and_records_explicit_pairs() {
    ...
    std::fs::create_dir_all(r.0.join(".engram")).unwrap();
    std::fs::write(r.0.join(".engram/searches.log"), "1000\thow does beta work\tfam/proj/alpha.md\n").unwrap();
    std::fs::write(r.0.join(".engram/reads.log"), "1010\tfam/proj/beta.md\n1020\tfam/proj/alpha.md\n5000\tfam/proj/beta.md\n").unwrap();
    let (ok, out, err) = engram(&r.0, &["learn"], "");
    assert!(ok, "{err}");
    assert!(out.starts_with("1 pair(s) learned"), "{out}");
    let table = std::fs::read_to_string(r.0.join(".engram/feedback.json")).unwrap();
    ...
    let (ok, _, _) = engram(&r.0, &["read", "alpha"], "");
    assert!(ok);
    let reads = std::fs::read_to_string(r.0.join(".engram/reads.log")).unwrap();
