...
fn corpus_texts() -> Vec<String> {
    ...
    let bench = std::env::var("KEPT_BENCH").map(PathBuf::from).unwrap_or_else(|_| kept::paths::state_dir(&root).join("bench-queries.json"));
...
fn bpe_tokenizer_matches_the_reference_everywhere() {
    ...
    let ours = kept::bpe::Bpe::from_file(&dir.join("tokenizer.json"), 8192).expect("bpe tokenizer");
    let mut reference = tokenizers::Tokenizer::from_file(dir.join("tokenizer.json")).expect("reference");
...
/// Every BPE model installed under `~/.kept/models` must match the reference
/// crate, whatever its split pattern and normaliser.
#[test]
fn every_installed_bpe_tokenizer_matches_the_reference() {
    let models = kept::paths::config_dir().join("models");
    ...
    for dir in dirs.flatten().map(|e| e.path()) {
        ...
        let ours = kept::bpe::Bpe::from_file(&json, 8192).unwrap_or_else(|e| panic!("{}: {e}", dir.display()));
