...
//! is what `engram search` shows; the second says whether the right passage was
//! the one put forward for that note.
//!
//! Queries come from a JSON file (`ENGRAM_BENCH`, default `<root>/.engram/bench-queries.json`)
...
//! `ENGRAM_NO_QUESTIONS=1` drops the question vectors, `ENGRAM_ID_BONUS=0` cuts the
//! lexical bonus, `ENGRAM_LEARN=0` cuts the learned bonus, `ENGRAM_LEXICAL=1` runs a
//! words-only baseline, `ENGRAM_CENTER=1` recenters vectors, `ENGRAM_MMR=<lambda>`
//! reranks by maximal marginal relevance.

use engrams::index::Index;
use engrams::similarity::rank_notes;
...
fn main() {
    ...
    let embedder = engrams::embedder::Embedder::load(&model).expect("model");

    let no_questions = std::env::var("ENGRAM_NO_QUESTIONS").is_ok();
    ...
    let center = std::env::var("ENGRAM_CENTER").is_ok();
    let mmr: Option<f32> = std::env::var("ENGRAM_MMR").ok().and_then(|v| v.parse().ok());
    let lexical = std::env::var("ENGRAM_LEXICAL").is_ok();
    ...
    let id_bonus: f32 = std::env::var("ENGRAM_ID_BONUS").ok().and_then(|v| v.parse().ok()).unwrap_or(0.04);
    ...
    for f in engrams::hot::notes_of(&root) {
        let rel = engrams::paths::relative(&f, &root);
        if let Ok(content) = std::fs::read_to_string(&f) {
            if let Some(name) = engrams::note::Note::parse(&content).field("name") {
                by_key.insert(engrams::check::link_key(name), rel.clone());
            }
        }
        by_key.insert(engrams::check::link_key(&f.file_stem().unwrap_or_default().to_string_lossy()), rel);
    }
    ...
    let learn = std::env::var("ENGRAM_LEARN").map_or(true, |v| v != "0");
    let table = engrams::feedback::Table::load(&state.join("feedback.json"));
    ...
    let learned = |v: &[f32], query: &str, base: &[engrams::similarity::NoteHit]| -> std::collections::HashMap<String, f32> {
        if !learn {
            return Default::default();
        }
        let b = engrams::feedback::learned_bonus(&table, today, |q| past.get(q).map(|pv| engrams::similarity::cosine(v, pv)), Some(query));
        let scores = base.iter().map(|h| (h.path.clone(), h.score)).collect();
        engrams::feedback::within_window(b, &scores)
    };
    let mut cases = cases;
    ...
    for (family, list) in &cases {
        ...
        for case in list {
            ...
            let hits = if lexical {
                ...
                match mmr {
                    None => engrams::similarity::rank_notes_with_bonus(&v, &chunks, 5, &bonus),
...
fn accepted_paths(base: &std::path::Path, path: &str, by_key: &std::collections::HashMap<String, String>) -> std::collections::HashSet<String> {
    ...
    for text in linked {
        ...
        while let Some(start) = rest.find("[[") {
            ...
            if let Some(p) = by_key.get(&engrams::check::link_key(target)) {
                out.insert(p.clone());
            }
            rest = &after[end + 2..];
        }
...
fn mmr_rerank(query: &[f32], chunks: &[(String, usize, Vec<f32>)], lambda: f32) -> Vec<engrams::similarity::NoteHit> {
    ...
    let mut chosen: Vec<engrams::similarity::NoteHit> = Vec::new();
    while chosen.len() < 5 && chosen.len() < pool.len() {
        ...
        for (i, cand) in pool.iter().enumerate() {
            ...
            let redundancy = chosen.iter().filter_map(|c| vec_of(&c.path, c.ordinal)).map(|v| engrams::similarity::cosine(cv, v)).fold(0f32, f32::max);
...
fn lexical_hits(base: &std::path::Path, query: &str) -> Vec<engrams::similarity::NoteHit> {
    ...
    for f in engrams::hot::notes_of(base) {
        ...
        scored.push((distinct, total, engrams::paths::relative(&f, base)));
    }
    scored.sort_by(|a, b| b.0.cmp(&a.0).then(b.1.cmp(&a.1)).then(a.2.cmp(&b.2)));
    scored.into_iter().take(5).map(|(d, _, path)| engrams::similarity::NoteHit { path, ordinal: 0, score: d as f32 }).collect()
}
