    ...
    let render_with = |masked: &std::collections::BTreeSet<Entry>| -> String {
        ...
        for (t, label) in &strata {
            ...
            if t == "project" && !masked.is_empty() {
                out.push(format!("- {} older project notes left out of the hot index, `kept search` finds them", masked.len()));
            }
            out.push(String::new());
        }
        ...
        if archived > 0 {
            out.push(format!("<!-- {archived} archived note(s), outside the index, reachable with kept search --archives -->"));
        }
